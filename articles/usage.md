...
4.  Copies the `R-validation.Rmd` file to the `R-validation` folder
...
6.  If the `create_zip` argument is set to `TRUE` and the validation is
...
1.  Creates a folder tree `R-validation/IQ-OQ-TestOutput` at the
    `path_save`.
