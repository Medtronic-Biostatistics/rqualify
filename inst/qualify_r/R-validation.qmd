...
tool_versions <- rqualify:::validation_tool_versions("quarto")
```
...
SystemResults <- res_summ$system_results[-1]
FileResults <- res_summ$test_results[-1]
IQResults <- res_summ$test_results[1]
```
...
The column labeled `System Results` records whether the individual test subprocess exited with status zero (`#statuscolor("PASS")`{=typst}) or returned a nonzero or unavailable exit status (`#statuscolor("FAIL")`{=typst}) in the execution of the program.
...
The result for **Installation Qualification** is listed in the `Test Results` column only. The result will be `#statuscolor("PASS")`{=typst} if the R subprocess exited successfully and emitted an explicit completion result, otherwise the result will be `#statuscolor("FAIL")`{=typst}.
